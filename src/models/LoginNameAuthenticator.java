...
public class LoginNameAuthenticator {
	...
	public int Validar_Senha(ArrayList<DuplaNumeros> dupla_numeros)
	{
		...
		if(Testa_Senhas(dupla_numeros, SALT, senha))
		{
	...
	private int Senha_Invalida()
	{
		...
		if(erros_senha <= 0)
		{
			...
			JOptionPane.showMessageDialog(null, "Usuario Bloqueado : Numero de tentativas maximo excedida", "Erro", JOptionPane.ERROR_MESSAGE);
			System.out.println("Usuario Bloqueado : Numero de tentativas maximo excedida");
	...
	public int Validar_ChavePrivada(String chave_privada_path, String frase_secreta)
	{
		...
		Signature assinatura = Gerar_AssinaturaDigital("SHA1withRSA");
	...
	public int Validar_Dados_Cadastro(String certificadoPath, String grupo_str, String senha_str, String senhaConfirma_str)
	{
		...
		{
			JOptionPane.showMessageDialog(null, "Senha precisa ser composta por 8 a 10 números e não são aceitas sequências de números repetidos", "Erro", JOptionPane.INFORMATION_MESSAGE);
	...
	public int Validar_Troca_Senha(String certificadoPath, String senha_str, String senhaConfirma_str)
	{
		...
		if(!senha_str.isEmpty() && !senhaConfirma_str.isEmpty())
		{
			...
			if(senha.compareTo(confirmaSenha) != 0)
			{
				JOptionPane.showMessageDialog(null, "Senha não bate com a confirmação", "Erro", JOptionPane.INFORMATION_MESSAGE);
				System.out.println("Senha não bate com a confirmação");
    ...
    private boolean Testa_Senhas(ArrayList<DuplaNumeros> dupla_numeros, String SALT, String senha_hash)
    {
    	String senha;
    	int[][] inputNum = new int[dupla_numeros.size()][2];
    	int tamanhoSenha = dupla_numeros.size();
    	
    	for(int h = 0; h < tamanhoSenha; h++)
    	{
    		inputNum[h][0] = dupla_numeros.get(h).num1;
    		inputNum[h][1] = dupla_numeros.get(h).num2;
    	}
    							...
    							for(int m = 0; m < 2; m++)
    							{
    								for(int n = 0; n < 2; n++)
    								{
    									for(int o = 0; o < 2; o++)
    									{
    										for(int p = 0; p < 2; p++)
    										{
		    									senha = String.valueOf(inputNum[0][i]);
		    									senha = senha + String.valueOf(inputNum[1][j]);
		    									senha = senha + String.valueOf(inputNum[2][k]);
		    									senha = senha + String.valueOf(inputNum[3][l]);
		    									senha = senha + String.valueOf(inputNum[4][m]);
		    									senha = senha + String.valueOf(inputNum[5][n]);
		    									senha = senha + String.valueOf(inputNum[6][o]);
		    									senha = senha + String.valueOf(inputNum[7][p]);
    									...
    									for(int o = 0; o < 2; o++)
    									{
    										for(int p = 0; p < 2; p++)
    										{
    											for(int q = 0; q < 2; q++)
    											{
			    									...
			    									senha = senha + String.valueOf(inputNum[6][o]);
			    									senha = senha + String.valueOf(inputNum[7][p]);
			    									senha = senha + String.valueOf(inputNum[8][q]);
			    									if(senha_hash.equals(Cryptografar_Senha(senha, SALT)))
			    										return true;
    											}
    										}
    										...
    										for(int p = 0; p < 2; p++)
    										{
    											for(int q = 0; q < 2; q++)
    											{
    												for(int r = 0; r < 2; r++)
    												{
				    									...
				    									senha = senha + String.valueOf(inputNum[7][p]);
				    									senha = senha + String.valueOf(inputNum[8][q]);
				    									senha = senha + String.valueOf(inputNum[9][r]);
				    									if(senha_hash.equals(Cryptografar_Senha(senha, SALT)))
				    										return true;
    												}
    											}
